...
float betaCalculation2(float beta, float sampleValue) {
...
float findBetaVal2(float * samples, float beta, float * outputArray) {
    ...
    for (int i = 0; i < sizeSamples; i++) {
        ...
        finalBetaVal = betaCalculation2(beta, samples[samplesCount]);
        if (finalBetaVal == -1 ) {
